...
// SCHEDULE_POLICY can be FIFO (0) or ROUND_ROBIN(1)
#undef SCHEDULE_POLICY_FIFO
#define SCHEDULE_POLICY_RR
...
void * cpu(void * arg) {
	int timestamp = 0;
    int *turnaround_time_arr = (int*) arg;
	...
	while (!load_done || !empty(&ready_queue)) {
		...
		if (proc == NULL) {
			...
			*/
			...
			// YOUR CODE HERE
#ifdef SCHEDULE_POLICY_FIFO
                exec_time = proc->burst_time;
#endif
#ifdef SCHEDULE_POLICY_RR
            ...
            {
                turnaround_time_arr[id] = timestamp - proc->arrival_time;
                free(proc);
            }
            else
            {
...
int main() {
	...
	load_task();
    int *turnaround_time = malloc(num_proc * sizeof(int));

	/* Start cpu */
	pthread_create(&cpu_id, NULL, cpu, (void*) turnaround_time);
	/* Start loader */
    ...
    for (int i = 0; i < num_proc; i++)
        printf("Turn around of process %2d: %3d\n", i, turnaround_time[i]);
