...
struct mem_region * find_best_region_fitting(unsigned int size, Strategy_t strat)
{
    struct mem_region *target = free_regions;
    struct mem_region *best_target = NULL;
    while (target != NULL)
    {
        if (target->size >= size)
        {
            if (best_target != NULL)
            {
                if (strat == BEST_FIT)
                {
                    best_target = (best_target->size < target->size) ?
                        best_target : target;
                }
    ...
    return best_target; // will be null if appropriate region not found
}

void * best_fit_allocator(unsigned int size) {
	// TODO: Implement your best fit allocator here
    struct mem_region *current_region = find_best_region_fitting(size, BEST_FIT);
    if (current_region != NULL)
    {
        struct mem_region* tmp =
...
void * worst_fit_allocator(unsigned int size) {
	// TODO: Implement your best fit allocator here
    struct mem_region *current_region = find_best_region_fitting(size, WORST_FIT);
    if (current_region != NULL)
    {
        struct mem_region* tmp =
            (struct mem_region*)malloc(sizeof(struct mem_region));
