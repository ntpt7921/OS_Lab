...
void* sum_worker (void *param);
long validate_sum(int arraysize);

/* Global sum buffer */
long sumbuf = 0;
int* shrdarrbuf;
pthread_mutex_t mtx = PTHREAD_MUTEX_INITIALIZER; // statically allocate the mutex

void* sum_worker (void *param) {
    struct _range *idx_range = (struct _range*) param;
    long localsum = 0;

    //printf("In worker from %d to %d\n", idx_range.start, idx_range.end);
    for (int i = idx_range->start; i <= idx_range->end; i++)
    {
        localsum += shrdarrbuf[i];
    }
